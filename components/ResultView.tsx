...
export const ResultView: React.FC<ResultViewProps> = ({ originalImage, data, onRetake, onDelete }) => {
  ...
  const groupedItems = useMemo(() => {
    ...
    if (!data.items || !Array.isArray(data.items)) return groups;
    ...
    try {
      ...
      const options: any = {
        ...
        onClone: (clonedNode: any) => {
          ...
          node.style.boxShadow = '0 10px 25px -5px rgba(0, 0, 0, 0.2)';
      ...
      const fileName = `japan-shopping-${data.date || 'record'}.png`;
  ...
  return (
    <div className="flex flex-col w-full max-w-md mx-auto animate-fade-in pb-24">
      ...
      <div className="relative drop-shadow-xl px-2">
        <div ref={contentRef} className="bg-white relative overflow-hidden text-slate-800 receipt-paper pb-8">
          ...
          <div className="pt-8 pb-6 px-6 text-center border-b-2 border-dashed border-slate-200">
            ...
            <p className="text-xs text-slate-400 tracking-widest uppercase mb-4">Japan Trip Report</p>
                 ...
                 {data.totalJpy && (
                    <div className="inline-block bg-white border border-slate-200 px-3 py-1 rounded-md">
                        <p className="text-xs text-slate-500 font-mono">
                            JPY: ¥{data.totalJpy.toLocaleString()} | Rate: {data.exchangeRate}
                        </p>
                    </div>
                 )}
             </div>
          ...
          <div className="p-6 space-y-8">
             {categories.length === 0 && (
                 <p className="text-center text-slate-400 text-sm italic">無商品明細</p>
             )}
                            ...
                            <div key={idx} className="flex justify-between items-start">
                                ...
                                <div className="text-right">
                                    <div className="text-sm font-bold text-slate-700 font-mono">
                                        ${(item.priceTwd || 0).toLocaleString()}
                                    </div>
                                    <div className="text-[10px] text-slate-400 font-mono">
                                        ¥{(item.originalPriceJpy || 0).toLocaleString()}
                                    </div>
                                </div>
                            </div>
                        ))}
          ...
          <div className="bg-slate-50 p-6 pt-8 text-center relative mt-4">
             ...
             {originalImage && (
                <div id="receipt-footer-image" className="hidden flex-col items-center gap-3 mt-6 pt-6 border-t border-slate-200">
                    <div className="text-xs text-slate-500 font-bold tracking-widest uppercase mb-1">Original Photo</div>
                    <img src={originalImage} alt="Receipt" className="w-full max-w-[80%] h-auto rounded grayscale opacity-80" />
                </div>
            )}
          </div>
      ...
      {originalImage && (
        <div className="mt-8 px-4 flex justify-center" data-hide-on-save="true">
          <details className="group w-full text-center">
            <summary className="list-none cursor-pointer inline-flex items-center justify-center gap-2 text-slate-400 hover:text-indigo-600 transition text-sm py-2 px-4 rounded-full border border-dashed border-slate-300">
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex items-center" data-hide-on-save="true">
        ...
        >
            ...
            <span className="font-bold tracking-wide">儲存/分享明細圖</span>
        </button>
      </div>
      ...
      <style>{`
        .receipt-paper {
            clip-path: polygon(
                0% 10px, 2% 0%, 4% 10px, 6% 0%, 8% 10px, 10% 0%, 12% 10px, 14% 0%, 16% 10px, 18% 0%, 20% 10px, 
                22% 0%, 24% 10px, 26% 0%, 28% 10px, 30% 0%, 32% 10px, 34% 0%, 36% 10px, 38% 0%, 40% 10px, 
                42% 0%, 44% 10px, 46% 0%, 48% 10px, 50% 0%, 52% 10px, 54% 0%, 56% 10px, 58% 0%, 60% 10px, 
                62% 0%, 64% 10px, 66% 0%, 68% 10px, 70% 0%, 72% 10px, 74% 0%, 76% 10px, 78% 0%, 80% 10px, 
                82% 0%, 84% 10px, 86% 0%, 88% 10px, 90% 0%, 92% 10px, 94% 0%, 96% 10px, 98% 0%, 100% 10px, 
                100% 100%, 0% 100%
            );
        }
      `}</style>
